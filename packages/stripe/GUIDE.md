...
Pass `config.stripe` as the second argument to `register`, like `@prefabs.tech/fastify-mailer` / `@prefabs.tech/fastify-graphql`. Omitting options or passing `{}` makes registration throw with `"Missing stripe configuration. Did you forget to pass it to the stripe plugin?"`.
...
Register with `register(stripePlugin, config.stripe)`. Calling `register(stripePlugin)` or `register(stripePlugin, {})` throws the `"Missing stripe…"` error.
